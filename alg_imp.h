...
/* Executes the power iteration algorithm to find the leading eigen vector
 ...
 * When the function ends, b_curr is the wanted leading eigen vector (normalized). */
void power_iteration(spmat* Ag, double* result, int M, int* g_ranks, double* f, double* b_curr, double* b_next);

/* Calculates the leading eigen value of B[g]_hat and return it.
